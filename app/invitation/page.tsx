  ...
  return (
    <div className="min-h-screen bg-[#060607] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-black/40 backdrop-blur-2xl border border-amber-900/30 rounded-3xl p-10 shadow-[0_20px_60px_-10px_rgba(217,119,6,0.15)] relative z-10 text-center">
        <h1 className="font-serif text-3xl text-white tracking-wide mb-2 font-light">The Nordic Manor</h1>
        ...
        <form action="https://formspree.io/f/xdapjqon" method="POST" className="space-y-5">
          ...
          <input type="hidden" name="Origine_Contatto" value="Waitlist Uomini" />
          <div className="text-left">
            <label htmlFor="email" className="block text-[10px] text-zinc-500 uppercase tracking-widest mb-2 pl-1">Accesso Prioritario</label>
            <input name="Email_Uomo" type="email" placeholder="Inserisci email" required className="w-full bg-black/50 border border-zinc-800 text-white rounded-xl px-5 py-4 focus:outline-none focus:border-amber-600 text-sm font-light" />
          </div>
          <button type="submit" className="w-full bg-amber-600 hover:bg-amber-500 text-black font-semibold tracking-widest py-4 px-4 rounded-xl transition-all shadow-lg text-xs uppercase">
            Unisciti alla Lista
          </button>
        </form>
      </div>
